...
const PRODUCTS = [
  { id:"p1",  name:"Bánh Kem Dâu Tây Mini",        price:"280.000", tags:["Hot","Bestseller"],  category:"mini",       rating:4.9, sold:234, img:CAKE_IMAGES[0] },
  { id:"p2",  name:"Bánh Sinh Nhật Hoa Hồng",      price:"350.000", tags:["Bestseller"],         category:"mini",       rating:4.8, sold:189, img:CAKE_IMAGES[1] },
  { id:"p3",  name:"Bánh Mousse Xoài Mango",        price:"320.000", tags:["New"],                category:"fruits",     rating:4.7, sold:156, img:CAKE_IMAGES[2] },
  { id:"p4",  name:"Bánh Bông Lan Trứng Muối Classic", price:"290.000", tags:["Hot"],            category:"saltedyolk", rating:4.9, sold:312, img:CAKE_IMAGES[3] },
  { id:"p5",  name:"Set Quà Tặng Premium",          price:"650.000", tags:["Đặc biệt"],          category:"gift",       rating:5.0, sold:88,  img:CAKE_IMAGES[4] },
  { id:"p6",  name:"Signature Velvet Hồng",         price:"480.000", tags:["Bestseller","Hot"],  category:"signature",  rating:4.9, sold:201, img:CAKE_IMAGES[5] },
  { id:"p7",  name:"Bánh Sinh Nhật Unicorn Bé",     price:"360.000", tags:["Cho bé","Hot"],      category:"kids",       rating:4.8, sold:143, img:CAKE_IMAGES[6] },
  { id:"p8",  name:"Bánh Tầng Sự Kiện Sang Trọng", price:"1.200.000",tags:["Sự kiện"],          category:"event",      rating:4.7, sold:54,  img:CAKE_IMAGES[7] },
  { id:"p9",  name:"Bánh Tết Hoa Mai",              price:"420.000", tags:["Ngày lễ"],           category:"holiday",    rating:4.8, sold:168, img:CAKE_IMAGES[8] },
  { id:"p10", name:"Bánh Matcha Hoa Quả Tổng Hợp", price:"340.000", tags:["New","Bestseller"],  category:"fruits",     rating:4.6, sold:127, img:CAKE_IMAGES[9] },
  { id:"p11", name:"Set Bánh Mini Combo 4",         price:"390.000", tags:["Hot"],               category:"mini",       rating:4.7, sold:178, img:CAKE_IMAGES[10] },
  { id:"p12", name:"Bánh Kem Choco Signature",      price:"450.000", tags:["Bestseller"],        category:"signature",  rating:4.9, sold:221, img:CAKE_IMAGES[11] },
];
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"40px", display:"flex", gap:48 }}>
        ...
        <div style={{ flex:"0 0 480px" }}>
          <div style={{ borderRadius:24, overflow:"hidden", boxShadow:C.cardShadow }}>
            <img src={product.img} alt={product.name} style={{ width:"100%", height:400, objectFit:"cover" }} />
          </div>
          <div style={{ display:"flex", gap:12, marginTop:12 }}>
            {[product.img, CAKE_IMAGES[2], CAKE_IMAGES[4]].map((img,i)=>(
              <div key={i} style={{ borderRadius:12, overflow:"hidden", flex:1, border:`2px solid ${i===0?C.accent:"transparent"}` }}>
...
const ProductFormModal = ({ product, onSave, onClose, title }) => {
  const [form, setForm] = useState({
    name: product?.name || "",
    price: product?.price || "",
    category: product?.category || "mini",
  ...
  const handleSave = () => {
    ...
    onSave({ ...product, ...form, tags, sold:Number(form.sold), rating:Number(form.rating), id: product?.id || "p"+Date.now() });
  };
  const inputStyle = { width:"100%", padding:"10px 14px", borderRadius:10, border:`1px solid ${C.pinkMid}`, fontSize:14, fontFamily:"inherit", outline:"none", boxSizing:"border-box" };
  return (
    <div style={{ position:"fixed", inset:0, zIndex:3000, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(0,0,0,0.45)", backdropFilter:"blur(8px)" }}>
      <Card style={{ width:520, padding:32, maxHeight:"85vh", overflowY:"auto", position:"relative" }}>
        <button onClick={onClose} style={{ position:"absolute", top:12, right:16, background:"none", border:"none", cursor:"pointer", fontSize:22, color:C.textMid }}>✕</button>
        <h2 style={{ fontFamily:"'Playfair Display', serif", color:C.deep, margin:"0 0 24px" }}>{title}</h2>
        ...
        <div style={{ marginBottom:14 }}>
          <label style={{ fontSize:12, fontWeight:700, color:C.deepText, display:"block", marginBottom:6 }}>Danh mục</label>
          <select value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))} style={{ ...inputStyle, background:"white" }}>
            {CATEGORIES.map(c=><option key={c.id} value={c.id}>{c.title}</option>)}
          </select>
        </div>
        <div style={{ marginBottom:14 }}>
          <label style={{ fontSize:12, fontWeight:700, color:C.deepText, display:"block", marginBottom:6 }}>Tags (phân cách bằng dấu phẩy: Hot, Bestseller, New)</label>
          <input value={form.tags} onChange={e=>setForm(f=>({...f,tags:e.target.value}))} style={inputStyle} placeholder="Hot, Bestseller, New" />
        </div>
        <div style={{ display:"flex", gap:12, marginBottom:14 }}>
          <div style={{ flex:1 }}>
            <label style={{ fontSize:12, fontWeight:700, color:C.deepText, display:"block", marginBottom:6 }}>Đã bán</label>
            <input type="number" value={form.sold} onChange={e=>setForm(f=>({...f,sold:e.target.value}))} style={inputStyle} />
          </div>
          <div style={{ flex:1 }}>
            <label style={{ fontSize:12, fontWeight:700, color:C.deepText, display:"block", marginBottom:6 }}>Đánh giá (0-5)</label>
            <input type="number" step="0.1" min="0" max="5" value={form.rating} onChange={e=>setForm(f=>({...f,rating:e.target.value}))} style={inputStyle} />
          </div>
        </div>
        <div style={{ marginBottom:14 }}>
          <label style={{ fontSize:12, fontWeight:700, color:C.deepText, display:"block", marginBottom:6 }}>URL hình ảnh</label>
          <input value={form.img} onChange={e=>setForm(f=>({...f,img:e.target.value}))} style={inputStyle} />
          {form.img && <img src={form.img} alt="preview" style={{ width:80, height:80, objectFit:"cover", borderRadius:8, marginTop:8, border:`1px solid ${C.pinkMid}` }}/>}
        </div>
        <div style={{ display:"flex", gap:12, marginTop:20 }}>
...
const AdminProducts = () => {
  ...
  const handleAdd = (newP) => { setProducts(ps=>[...ps, newP]); setShowAdd(false); };
  const handleEdit = (updatedP) => { setProducts(ps=>ps.map(p=>p.id===updatedP.id?updatedP:p)); setEditProduct(null); };
  const handleDelete = (id) => { if(window.confirm("Bạn có chắc muốn xóa sản phẩm này?")) setProducts(ps=>ps.filter(x=>x.id!==id)); };
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:12, marginBottom:20 }}>
        <input value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} placeholder="🔍 Tìm sản phẩm..." style={{ flex:1, padding:"10px 18px", borderRadius:50, border:`1px solid ${C.pinkMid}`, fontSize:14, fontFamily:"inherit", outline:"none", background:C.white }} />
        <select value={filterCat} onChange={e=>setFilterCat(e.target.value)} style={{ padding:"10px 18px", borderRadius:50, border:`1px solid ${C.pinkMid}`, fontSize:14, fontFamily:"inherit", outline:"none", background:C.white }}>
          <option value="all">Tất cả danh mục</option>
          {CATEGORIES.map(c=><option key={c.id} value={c.id}>{c.title}</option>)}
        </select>
        <span style={{ fontSize:13, color:C.textMid, display:"flex", alignItems:"center" }}>{filtered.length} sản phẩm</span>
      </div>
      <Card style={{ padding:0, overflow:"hidden" }}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <thead style={{ background:C.pinkLight }}>
            <tr>{["Sản phẩm","Danh mục","Giá","Đã bán","Tags","Thao tác"].map(h=>(
              ...
              <tr key={p.id} style={{ borderTop:`1px solid ${C.pinkLight}`, background:i%2===0?"white":C.cream }}>
                <td style={{ padding:"12px 16px" }}>
                  <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                    <img src={p.img} alt="" style={{ width:44, height:44, objectFit:"cover", borderRadius:8 }} onError={e=>{e.target.style.display='none'}} />
                    <span style={{ fontSize:13, fontWeight:700, color:C.deepText }}>{p.name}</span>
                  </div>
                </td>
                <td style={{ padding:"12px 16px", fontSize:13, color:C.textMid }}>{CATEGORIES.find(c=>c.id===p.category)?.title}</td>
                <td style={{ padding:"12px 16px", fontSize:13, fontWeight:700, color:C.accent }}>{p.price}đ</td>
                <td style={{ padding:"12px 16px", fontSize:13, color:C.textMid }}>{p.sold}</td>
                <td style={{ padding:"12px 16px" }}><div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>{p.tags.map(t=><Tag key={t} label={t}/>)}</div></td>
                <td style={{ padding:"12px 16px" }}>
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={()=>setEditProduct(p)} style={{ padding:"6px 14px", borderRadius:50, border:`1px solid ${C.accent}`, background:"white", cursor:"pointer", color:C.accent, fontSize:12, fontWeight:600, fontFamily:"inherit" }}>Sửa</button>
                    <button onClick={()=>handleDelete(p.id)} style={{ padding:"6px 14px", borderRadius:50, border:"1px solid #e74c3c", background:"white", cursor:"pointer", color:"#e74c3c", fontSize:12, fontWeight:600, fontFamily:"inherit" }}>Xóa</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {showAdd && <ProductFormModal title="Thêm Sản Phẩm Mới" onSave={handleAdd} onClose={()=>setShowAdd(false)} />}
      {editProduct && <ProductFormModal title="Chỉnh Sửa Sản Phẩm" product={editProduct} onSave={handleEdit} onClose={()=>setEditProduct(null)} />}
    </div>
  );
};
